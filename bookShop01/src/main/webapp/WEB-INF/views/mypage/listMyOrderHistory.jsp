...
function fn_cancel_order(order_id){
	var answer=confirm("구독을 연장하시겠습니까?");
	if(answer==true){
		var formObj=document.createElement("form");
          ...
          <c:when test="${item.order_id != pre_order_id }">   
            <tr>       
				...
				<td>
			     <c:choose>
			   <c:when test="${item.delivery_state=='delivery_prepared'}">
			       <input  type="button" onClick="fn_cancel_order('${item.order_id}')" value="구독취소"  />
			   </c:when>
			   <c:otherwise>
			      <input  type="button" onClick="fn_cancel_order('${item.order_id}')" value="구독취소" disabled />
			   </c:otherwise>
			  </c:choose>
			    </td>
			</tr>
